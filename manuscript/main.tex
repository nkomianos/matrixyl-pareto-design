...
The 9-member frontier breaks into three regions by trade-off character. PTTPS and KTTPP, both at edit distance 2, give the largest gain in the penetration descriptor (67\%--69\%) but retain only 67\% of the motif-preservation score. KTTPS and KTTKP, at edit distance 1, give a more modest gain (38\%--50\%) while preserving 83\%--84\% of the motif. KTTKS itself is the lower endpoint: 0\% gain but full motif preservation, included by definition.

A three-arm experimental design follows directly. KTTPS, the single-mutation candidate, has the highest prior probability of retaining biological activity and is the natural primary candidate. PTTPS is the aggressive arm: it tests whether the descriptor gain at the limit translates into measurable permeation, accepting a larger functional cost. KTTKS serves as the unmodified positive control.
...
We applied a deterministic computational workflow (tournament GA, NSGA-II, RDKit descriptors, conformer-ensemble validation, and a sensitivity sweep over penalty weights) to the Matrixyl core peptide KTTKS and obtained a 9-member Pareto frontier in the trade-off between predicted skin permeability and motif preservation. The frontier is identical to the ground-truth Pareto set obtained by exhaustive enumeration of the 3,706-candidate search space.

The dominant substitution on the frontier is lysine-to-proline at position 4. It recurs across all $\pm 30\%$ perturbations of the descriptor weights and is interpretable directly in terms of charge, polarity, and hydrogen-bond count. We propose three candidates for experimental validation: KTTPS (single mutation, balanced trade-off) as the primary candidate, PTTPS (two mutations, larger predicted gain) as the aggressive arm, and KTTPP as a backup. All code, results, and figures are openly available, and 48 unit tests cover the descriptor calculations, the search-space rules, and the optimizer implementations.
